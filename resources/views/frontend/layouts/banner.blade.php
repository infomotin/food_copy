    ...
    <div class="container">
        ...
        <div class="mx-auto col-lg-8">
            ...
            <div class="owl-carousel owl-carousel-category owl-theme">
                @php
                    $products = App\Models\Client\Product::where('status','active')->take(5)->get();
                @endphp
                @foreach ($products as $product)
                <div class="item">
                    <div class="osahan-category-item">
                    <a href="#">
                        <img class="img-fluid" src="{{asset('upload/products/'.$product->image)}}" alt="{{ $product->name }}">
                        <h6>{{ Str::limit($product->name, 10, '...') }} </h6>
                        <p>{{ $product->price }}</p>
                    </a>
                    </div>
                </div>
